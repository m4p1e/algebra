...
\begin{enumerate}
	...
	\item $x$不是$D$的lower bound，$y$不是$D$lower bound. 那么$f(x) = \max D(x)$和$f(y) = \max D(y)$. 若$x \nleq d$，则$y \nleq d$ (else $x \leq \leq d$). 所以$D(x) \subseteq D(y)$. 由于$D(y)$是一个降链上点的集合，这个集合还是一个降链，你在降链上找子集$D(x)$，肯定有$\max D(x) \leq \max D(y)$，则$f(x) \leq f(y)$.
\end{enumerate}
