...
{\color{blue} 下面的定理在说如果有一个complete lattice和closure operator之后可以得到一个新的同构的lattice}.
...
\begin{definition}
\rm A lattice $\lattice$ is said to be algebraic, or compactly generated, if it is complete and $\lattice^c$ is join dense in $\lattice$, i.e., $x = \bigvee (\downarrow x \cap L^c)$ for every $x \in L$.
\end{definition}

{\color{blue} 换句话说就是在$\lattice^c$中比$x$小的它们的join是$x$}. 
\end{document}