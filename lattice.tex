...
反过来如果$\sum$是一族finitary closure rules，那么也是可以确定一堆closed set，相似的用上面的方法去构造一个closure operator $\Gamma_{\sum}$，现在我们要来说明它是algebraic. 给定$B \subseteq X$，若$x \in \Gamma_{\sum}(B)$，那么对于每一个closed set $S\supseteq B$都有$x \in S$. 那么按照$F \subseteq S \Rightarrow z \in S$，则每一个$S$都包含同一个finite set $F$与之对应，考虑$F$与$B$的关系
\begin{enumerate}
	\item 若$F \subseteq B$，那么刚刚好$x \in \Gamma(F)_{\sum}$.
	\item 若$F \nsubseteq B$，那么考虑finite set $F - B$，每一个$x \in F - B $，也应该对应一个finite set $F^{-1} \subseteq S$...
\end{enumerate}
为了用上面的方法，你得证明$\Gamma_{\sum}(B)$是$B$靠着closure rules生成的. 这里的证明去看universe alegbra中证明$Sg$是一个algebraic closure operator的方法.
\end{proof}

\newpage
\begin{theorem}
\rm Let $\Gamma$ be an algebraic closure operator on set $X$. Then $\mathcal{C}_\Gamma$ is an algebraic lattice whose compact elements are $\SET{\Gamma(F)}{$F$ is a finite subset of $X$}$.
\end{theorem}
...
反过来，若$C$是$\mathcal{C}_{\Gamma}$上的compact element. {\color{red}我们要证明$C$是$X$上某个有限子集的闭包}. 自然地$C$是closed的，则$\Gamma(C) = C$，因为$\Gamma$是algebraic的，那么$C = \bigcup\SET{\Gamma(F)}{$F$ is finite subset of $C$}$. 又因为$C$是compact的，所以存在有限多个$F_1,\cdots,F_n$使得$C = F_1 \vee \cdots \vee F_n = \Gamma(F_1 \cup \cdots \cup F_n).$命题得证.

{\color{red} 最后证明$\mathcal{C}_{\Gamma}^c$是join dense的}. 对于任意的$A \subseteq \mathcal{C}_{\Gamma}$，$A$是closed，那么$A = \bigvee\limits_{i \in I} \Gamma(F_i)$其中finite subset$F \subseteq A$. 自然地有$\bigvee(\downarrow A \cap \mathcal{C}_{\Gamma}^c) \leq A$且$\Gamma(F_i)_{i \in I} \in \mathcal{C}_{\Gamma}^c$，那么$\bigvee(\downarrow A \cap \mathcal{C}_{\Gamma}^c) = A$. 命题得证.
\end{proof}
...
\newpage 
\begin{definition}
\rm Let $\slattice = (S, \vee)$ be a join semilattice. A subset $A$ of $S$  is called an ideal if
\begin{enumerate}
...
\begin{proposition}
\rm Ideals are defined by closure rules，so the intersection of a set of ideals of $\slattice$ is again one. Since the closure rules are finitary, the lattice of ideals is algebraic. 
...
{\color{blue} 前面说$\lattice^c$其实已经是一个join semilattice with a least element，想让它变成一个complete lattice，你得让join操作不仅仅在finite下成立}.
