...
\begin{proof}
这个命题在1980被Pavel Pavel Pudlák and Jı́ři Tůma证明了，but the proof is quite difficult，{\color{blue} 若是未来机会再去看看吧}.
\end{proof}
...
$$
首先我们想证明$h(a)$是一个subgroup of $\mathcal{G}$. 若存在$\pi_1, \pi_2 \in h(a)$和任意的$x \in X$，考虑$\pi_1 \circ \pi_2(x) = \pi_1(\pi_2(x))$，那么$x~F(a)~\pi_2(x)~ F(a)~\pi_1(\pi_2(x))$，所以$\pi_1 \circ \pi_2 \in h(x)$. $h$是monotone还是比较显然的. 最后的lattice homomorphism证明，若$a , b \in L$, 考虑$h(a \wedge b)$， 在monotone的条件下我们只需要证明$h(a \wedge b) \geq h(a) \wedge h(b) = h(a) \cap h(b)$. 若$\pi \in h(a) \cap h(p)$，即$(x,\pi(x) \in F(a)$和$(x,\pi(x)) \in F(b)$，那么$(x,\pi(x) \in F(a) \wedge F(b))$，所以$h(a) \cap h(b) \in h(a\wedge b)$.
