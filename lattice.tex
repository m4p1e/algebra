...
{\color{red} By convention，把$\bigcap \emptyset = X$的话，closure system配合集合操作就是一个complete lattice}.
...
\begin{theorem}
\rm If $\Gamma$ is a closure operator on a set $X$，$\sum_{\Gamma}$ be the set of (1)all rules where $c \in \Gamma(\emptyset)$，and (2)all rules
$$
Y \subseteq  S \Rightarrow z \in S
$$
...
{\color{blue} 说实话上面这个命题有点abstract，当有一个closure operator之后，我们知道closed set在它的作用下是它本身，但是能不能找到一个closure rule来和它对应呢}?

\begin{theorem}
\rm If $\sum$ is a set of closure rules on set $X$，let $\mathcal{C}_{\sum}$ be the collection all subsets of $X$ that satisfy all the rules of $\sum$. Then a set $\mathcal{C}_{\sum}$ is a closure system. 
\end{theorem}
