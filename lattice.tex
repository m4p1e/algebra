...
%反过来如果$\sum$是一族finitary closure rules，我们定义这样一个closure operator $\Gamma_{\sum}$，给定$B \subseteq X$，若$x \in \Gamma_{\sum}(B)$当且仅当finite $F \subseteq B$，使得$x \in \Gamma_{\sum}(F)$. 显然这样定义出来的 那么$\Gamma_{\sum}$就是algebraic.
\end{proof}
