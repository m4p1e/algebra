...
{\color{blue} 这一章我们的主旋律，先给出closure system是什么？ 在告诉你如何构造第一个closure system(closure rules)？ 再告诉你如何在一个closure system上取一个合适的元素出来(closure operator)？ 慢慢享受吧 ） 
\begin{center}
...
\begin{proof}
第一类条件，即对任意的subset$A$，有$F(\emptyset) \subseteq \Gamma(A \cup \emptyset)$. 第二类条件，即如果满足规则$Y \subseteq S \Rightarrow s \in S$，那么显然有$s \in \Gamma(S)$，还因为$Y \subseteq Y$，那么也有$s \in \Gamma(Y)$. 显然这些这些规则是非常的自然的，那些固有的元素放在了$F(\emptyset)$中，因为所有子集都包含空集，而根据具体子集需要添加的元素根据所给的规则添加即可. 
