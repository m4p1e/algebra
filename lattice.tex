...
反过来如果$\sum$是一族finitary closure rules，那么也是可以确定一堆closed set，相似的用上面的方法去构造一个closure operator $\Gamma_{\sum}$，现在我们要来说明它是algebraic. 你得证明$\Gamma_{\sum}(B)$是$B$靠着closure rules生成的. 这里的证明去看universe alegbra中证明$Sg$是一个algebraic closure operator的方法.
\end{proof}
...
{\color{blue} 之前我们有从closure operator生成complete lattice的过程，现在我们也可以自然地从algebraic closure operator生成一个algebraic lattice}.
...
\begin{proposition}
\rm Ideals are defined by closure rules，so the intersection of a set of ideals of $\slattice$ is again one. Since the closure rules are finitary, {\color{red} the lattice of ideals is algebraic}. 
