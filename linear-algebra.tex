...
\begin{proposition}
给定finite-dimensional $V$ 和它的subspace $U$. 则
$$
\dim U + \dim U^0 = \dim V.
$$
\end{proposition}
...
\begin{proof}
RHS可以换成$\dim V'$. 如果我们想用fundamental theorem of linear map. 要把$U$弄进来，必须要linear map的一端是$U$. 所以这里引入inclusion map $i \in \mathcal{U,V}$，定义为
$$
i(u) = u.
$$ 
那么它的dual map $\func{i'}{V'}{U'}$. 我们把fundamental theorem用在dual map上有
$$
\dim V' = \dim\nul{i'} + \dim\range{i'}. 
$$
\end{proof}
