...
这个定义的直觉是什么呢？当给定一个linear map $\func{T}{V}{W}$和$V$上一个basis $v_1,\cdots,v_n$. $Tv_1,\cdots,Tv_n$是$\range{T}$的一个basis，所以它们spans $\range{T}$. 这里的想法就是把$Tv_1,\cdots,Tv_n$和$W$上的basis联系起来了.
...
但是在此之前，我们并没有定义矩阵乘法. 如果我们希望上面的问题有一个positive answer，定义$\mathcal{M}(S) = A$和$\mathcal{M}(S) = C$，取$1 \leq k \leq p $，那么
$$
\begin{aligned}
